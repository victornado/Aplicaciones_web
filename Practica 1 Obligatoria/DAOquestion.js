    ...
    getAllQuestions(callback){
        this.pool.getConnection(function(err, connection) {
            if (err) { 
                ...
                connection.query("SELECT q.id, q.title, q.text, q.date, u.icon,t.tag, u.nick from question q join users u on u.nick = q.nick left join tags t on t.idQuestion=q.id order by q.title " ,
                function(err, rows) {
                    connection.release(); // devolver al pool la conexión
                    if (err) {
                        callback(new Error("Error de acceso a la base de datos"),null);
    ...
    searchByTextBar(text,callback){
        this.pool.getConnection(function(err, connection) {
            if (err) { 
                ...
                connection.query("SELECT q.title, q.text, q.date, u.icon, t.tag, u.nick from question q join users u on u.nick=q.nick join tags t on t.idQuestion=q.id where instr( q.title , ? ) or instr( q.text , ? ) order by q.title;" ,
                [text, text],
                function(err, rows) {
                    connection.release(); // devolver al pool la conexión
                    if (err) {
